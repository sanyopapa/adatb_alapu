...
<nav class="navbar">
    ...
    <?php
      ...
      <div class="dropdown">
        ...
        <a class="active_dropdown" href="refresh_others_page.php">Adatok módosítása</a>
...
<form action="process.php" method="POST" enctype="multipart/form-data">
  ...
  <form id="form-login" action="refresh_others_check.php" method="POST">
            <fieldset class="form_2">
                <legend>Egyéb adatok módosítása</legend>
                <?php 
                ...
                if ($_SESSION["mod"]==0 && $_SESSION["admin"]==0) {echo '<label for="" class="empty">Az e-mail cím módosításához keress fel egy moderátort!</label><br>';}
                else {
                echo '<label for="email">E-mail cím: </label>';
                echo '<input type="email" id="email" name="email" size="25" placeholder=" '.$_SESSION["email"].'"><br>';
                }
                echo '<br>';
                echo '<label for="user-name">Felhasználónév: </label>';
                echo '<input type="text" id="user-name" name="user-name" size="25" placeholder=" '.$_SESSION["user_name"].'"><br><br>';
                echo '<label for="birth">Születési dátum: </label>';
                echo '<input type="date" id="birth" name="birth" min="1920-01-01" placeholder="'.$_SESSION["birth"].'"/> <br><br>';
                echo '';
                echo '<label for="intro">Bemutatkozás:</label><br>';
                echo '<textarea name="intro" rows="5" cols="45" maxlength="200" placeholder="'.$_SESSION["intro"].'"></textarea><br>'; 
